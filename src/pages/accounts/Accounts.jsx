              ...
              {/* <div className='flex justify-between items-center flex-col lg:flex-row gap-4'>
                <div className='flex items-center gap-4 justify-between w-full lg:justify-start'>
                  ...
                  <div className='list__icons'>
                    <div className='list__icon' data-tooltip-id="accoutnTooltip" data-tooltip-content="Print"
                      onClick={() => exportTable('print')}>
                      <BiPrinter className='text-white text-[16px]' />
                    </div>
                    <div className='list__icon' data-tooltip-id="accoutnTooltip" data-tooltip-content="Copy Table"
                      onClick={() => exportTable('copy')}>
                      <FaRegCopy className='text-white text-[16px]' />
                    </div>
                    <div className='list__icon' data-tooltip-id="accoutnTooltip" data-tooltip-content="Download PDF"
                      onClick={() => exportTable('pdf')}>
                      <FaRegFilePdf className='text-white text-[16px]' />
                    </div>
                    <div className='list__icon' data-tooltip-id="accoutnTooltip" data-tooltip-content="Download Excel"
                  ...
                  <tbody>
                    {
                      billData.map((data, i) => {
                        return <tr key={i}>
                          ...
                          <td className='px-4 border-b' align='center'>{data.type}</td>
                          <td className='px-4 border-b' align='center'>{data.openingBalance}</td>
                          <td className='px-4 border-b max-w-[60px]'>
                            <div className='flex flex-col md:flex-row gap-2 mr-2 justify-center'>
                              <button
                                data-tooltip-id="accoutnTooltip" data-tooltip-content="Edit"
                                onClick={() => navigate(`/admin/account/edit/${data._id}`)}
                                className='bg-blue-400 text-white px-2 py-1 rounded w-[35px] text-[16px] flex justify-center'>
                                <MdEditSquare />
                              </button>
                              <button
                                data-tooltip-id="accoutnTooltip" data-tooltip-content="Details"
                                onClick={() => navigate(`/admin/bill/details/${data._id}`)}
                                className='bg-red-500 text-white px-2 py-1 rounded w-[35px] text-lg flex justify-center'>
                                <IoInformationCircle />
                              </button>
                            </div>
                          </td>
